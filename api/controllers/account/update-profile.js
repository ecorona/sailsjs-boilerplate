module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    if (_.findIndex(['beginChange', 'changeImmediately', 'modifyPendingChange'], desiredEffectReEmail)>-1) {
    ...
    //si cambio el telefono...
    var newTelefono = inputs.telefono;
    if (newTelefono !== undefined) {
      newTelefono = newTelefono.toLowerCase();
    }

    if(newTelefono !== this.req.me.telefono){ //el telefono ha cambiado!
      var token = parseInt(Math.random()*1000000)+'';
      valuesToSet.telefono = newTelefono;
      valuesToSet.telefonoStatus = 'unconfirmed';
      if(newTelefono){
        valuesToSet.tokenTelefono = token;
        notificaService.sms(newTelefono, sails.config.custom.app.name+'\n\nHola '+this.req.me.fullName+'. \n\nTu código de verificación celular es: '+token);
      }
    }
